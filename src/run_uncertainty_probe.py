...
def _load_arrays(data_dir):
    test_x = np.load(os.path.join(data_dir, "test_x.npy"), mmap_mode="r")
    test_y = np.load(os.path.join(data_dir, "test_y.npy"), mmap_mode="r")
...
def main():
    ...
    save_dir = os.path.join(args.save_root, f"uncertainty_probe_{timestamp}")
    os.makedirs(save_dir, exist_ok=True)

    print("加载数据...")
    test_x, test_y, train_x, train_y = _load_arrays(args.data_dir)
