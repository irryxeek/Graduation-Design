...
image_mappings = [
    {
        "target": "数据处理流水线",
        "path": r"D:\02_Study\01_Schoolwork\Graduation Design\docs\midterm\数据处理流程图.png",
        "caption": "图 2.1 FY-3D GNOS L2 ATP 数据预处理流水线",
        "width": 5.5
    },
    {
        "target": "气压对数变换的关键作用",
        "path": r"D:\02_Study\01_Schoolwork\Graduation Design\docs\midterm\pressure_dist_comparison.png",
        "caption": "图 2.2 气压数据对数变换前后的分布特征对比",
        "width": 5.8
    },
    {
        "target": "训练过程分析",
        "path": r"D:\02_Study\01_Schoolwork\Graduation Design\docs\midterm\loss_curve.png",
...
def add_image_after_paragraph(para, img_path, caption, width_inches):
    # Insert a new paragraph using the docx method
    # Note: insert_paragraph_before() is more stable than manual OXML for alignment
    # We will find the paragraph index and insert after it
    pass

# We'll use a safer approach: find the paragraph, and insert before the NEXT paragraph
# If it's the last paragraph, we just add_paragraph at the end.
...
for mapping in image_mappings:
    ...
    if found_idx != -1:
        ...
        # Paragraph to insert before is found_idx + 1
        if found_idx + 1 < len(doc.paragraphs):
            next_p = doc.paragraphs[found_idx + 1]
            
            # 1. Insert caption (it will be below the image)
            ...
            # 2. Insert image (above the caption)
            img_para = cap_para.insert_paragraph_before("")
            img_para.alignment = WD_ALIGN_PARAGRAPH.CENTER
            img_run = img_para.add_run()
            img_run.add_picture(mapping["path"], width=Inches(width_inches))
        else:
            # Last paragraph case
            ...
            cap_para.alignment = WD_ALIGN_PARAGRAPH.CENTER
            # ... (skip font for last para brevity or add it)
    else:
